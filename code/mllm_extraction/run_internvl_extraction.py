...
OUTDIR = "/content/drive/MyDrive/vllm - experiment/outputs_internvl"
...
# Controls how many tiles each image is split into (higher = more detail, more VRAM). Default: 4
MAX_DYNAMIC_PATCH = 8
MAX_IMAGES_PER_PROMPT = 6

# Stop tokens for InternVL — kept out of SamplingParams default so they can be reused consistently.
INTERNVL_STOP_TOKENS = ["<|endoftext|>", "<|im_start|>", "<|im_end|>"]
...
def load_internvl3_5(image_path, strat, dataset_name, tokenizer):
    """Build a ModelRequestData for a single target image under `strat`."""
    ...
    prompt_key = strat + '_uml' if 'uml' in dataset_name else strat
    ...
    placeholders = "\n".join(
        f"Image-{i}: <image>" for i, _ in enumerate(path2images, start=1)
    )
    full_question = f"{placeholders}\n{question}"
    ...
    return ModelRequestData(
        prompt=prompt,
        image_data=[Image.open(p).convert("RGB") for p in path2images],
    )


def get_batch(strat, dataset_name, tokenizer):
    imgfiles = sorted(os.listdir(os.path.join(BASE_DIR, dataset_name, 'test')))
    prompt_batch = []
    for img_name in imgfiles:
        req = load_internvl3_5(
...
def run_generate(llm):
    ...
    stop_token_ids = [tokenizer.convert_tokens_to_ids(t) for t in INTERNVL_STOP_TOKENS]
    ...
    for t in [0.1]:
        ...
        for strat in ['zero_shot', 'two_shot', 'five_shot']:
            for d in ['synth_fc', 'synth_dir_graphs']:

                for i in range(N_RUNS):
                    out_dir = os.path.join(OUTDIR, d, strat)
                    os.makedirs(out_dir, exist_ok=True)

                    extraction_prompts, image_names = get_batch(strat, d, tokenizer)
