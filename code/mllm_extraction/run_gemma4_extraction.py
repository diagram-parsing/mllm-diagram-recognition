...
OUTDIR = "/content/drive/MyDrive/vllm - experiment/outputs_gemma4_givenNodes"
...
def load_gemma4(image_path, strat, dataset_name, processor):
    ...
    if strat == 'one_shot':
        ...
        with open(f'./labels/{dataset_name}/{example_labels[0]}') as f:
            ...
            question = question.replace('<node_names>', str(label_data['nodes']))
        ...
        for i in range(2):
            ...
            with open(f'./labels/{dataset_name}/{example_labels[i]}') as f:
                ...
                question = question.replace(f'<node_names{i+1}>', str(label_data['nodes']))
    ...
    target_label_path = f'{BASE_DIR_LABELS}/{dataset_name}/test/{target_stem}.json'

    if not os.path.exists(target_label_path):
        target_nodes = "No information on the node names available"
    else:
        with open(target_label_path) as f:
            target_label_data = json.load(f)
            target_nodes = str(target_label_data['nodes'])


    shot_count = {'zero_shot': 0, 'one_shot': 1, 'two_shot': 2, 'five_shot': 5}
    n = shot_count.get(strat, 0)
    if n == 0:
        question = question.replace('<node_names>', target_nodes)
    else:
        question = question.replace(f'<node_names{n+1}>', target_nodes)

    path2images.append(image_path)

    content = [{"type": "image"} for _ in path2images]
...
def run_generate(llm):
    ...
    for t in [0.1]:
        ...
        for strat in ['zero_shot', 'two_shot', 'five_shot']:
            for d in ['synth_dir_graphs', 'synth_fc']:
